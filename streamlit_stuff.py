...
option = st.sidebar.selectbox('Select the type of data to be processed', ('Select an Option', 'CSV File', 'Images'))
file = None
if option == 'CSV File':
    ...
    if len(image_files) > 0:
        ...
        threshold1 = st.sidebar.slider('Minimum Threshold', min_value=0, max_value=200, help='Any pixel below this threshold is eliminated, and any above are consider possible edges')
        threshold2 = st.sidebar.slider('Definite Threshold', min_value=0, max_value=200, help='Any pixel above this threshold is consider a definite edge. Additionally any pixel above the minimum threshold and connected to a pixel already determined to be an edge will be declared an edge')
        blur = st.sidebar.slider('blur', min_value=1, max_value=5, help='Filters out noise. Note: blur values must be odd so blur_value = 2 x slider_value + 1')

        (contours, _), edge_img = find_contours(image, threshold1=threshold1, threshold2=threshold2, blur=2*blur-1)
        image_copy = cv2.drawContours(image=image_copy, contours=contours, contourIdx=-1, color=(100, 200, 255), thickness=1, lineType=cv2.LINE_AA)
